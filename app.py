...
def get_latest_rev_info(row):
    """여러 리비전 열 중 최신 데이터 하나만 추출하여 단일화 (원상 복구)"""
...
def main():
    ...
    for i, tab in enumerate(tabs):
        with tab:
            ...
            # --- 4. UPLOAD MODAL SECTION (Save & Change 포함) ---
            if st.session_state.get(up_toggle, False):
                with st.container(border=True):
                    st.markdown("### 📄 최신 Drawing List 업데이트")
                    f = st.file_uploader("파일을 드래그하여 업로드하세요.", type=['xlsx'], key=f"file_{i}")
