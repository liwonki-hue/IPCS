...
def apply_professional_style():
    """Applies global CSS for fixed headers, compact buttons, and text wrapping."""
    st.markdown("""
        <style>
        ...
        /* [CRITICAL] Enable Text Wrapping for Dataframe Cells */
...
def open_duplicate_manager(df):
    ...
    st.warning("Action Required: Purging will retain only the latest entry (bottom-most).")
...
def open_upload_dialog():
    st.info("Upload new drawings to merge or replace the master database.")
...
def show_doc_control():
    ...
    # --- [Key Fix] Explicit Column Configuration for Visibility and Wrapping ---
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Description": st.column_config.TextColumn("Description", width="large"), # Guaranteed visibility
            "Rev": st.column_config.TextColumn("Rev", width="small"),
            "Date": st.column_config.TextColumn("Date", width="small"),
            "Hold": st.column_config.TextColumn("Hold", width="small"),
            "Status": st.column_config.TextColumn("Status", width="small"),
            "Remark": st.column_config.TextColumn("Remark", width="large") # Enabled for wrapping
        }
    )
