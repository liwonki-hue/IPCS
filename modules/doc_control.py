...
def apply_compact_button_ui():
    """필터 및 액션 버튼 사이즈 축소와 데이터 18px 유지 CSS"""
    st.markdown("""
        <style>
        ...
        .block-container { padding: 0.8rem 2.5rem !important; }
        [data-testid="stHeader"] { display: none !important; }
        
        /* 1. 섹션 레이블 및 타이틀 축소 */
        .main-title { font-size: 22px !important; font-weight: 800; color: #1657d0; margin-bottom: 8px; }
        .section-label { 
            font-size: 11px !important; 
            font-weight: 700; color: #6b7a90; text-transform: uppercase; margin-bottom: 3px; 
        }

        /* 2. Revision Filter & Action 버튼 사이즈 축소 (높이 28px, 폰트 12px) */
        div.stButton > button {
            border-radius: 2px; border: 1px solid #dde3ec;
            background-color: white; color: #374559;
            height: 28px !important; /* 한 단계 더 축소 */
            font-size: 12px !important; /* 폰트 축소 */
            padding: 0 8px !important;
            line-height: 1 !important;
        }
        div.stButton > button[kind="primary"] { background-color: #0c7a3d !important; color: white !important; }

        /* 3. Search & Filter 입력창 컴팩트화 */
        ...
        /* 5. 결과 요약 텍스트 축소 */
        .result-info { font-size: 13px; color: #374559; font-weight: 600; padding-top: 4px; }
        </style>
    """, unsafe_allow_html=True)

def show_doc_control():
    apply_compact_button_ui()
    st.markdown("<div class='main-title'>Drawing Control System</div>", unsafe_allow_html=True)

    if not os.path.exists(DB_PATH):
        st.error(f"Error: Database file not found.")
        return
    ...
    rev_cols = st.columns(14) # 열 개수를 늘려 버튼 가로폭 축소
