...
def apply_professional_style():
    """Enhanced CSS for ensuring text wrap and header stability."""
    st.markdown("""
        <style>
        ...
        div.stButton > button, div.stDownloadButton > button {
            border-radius: 4px; border: 1px solid #dde3ec; height: 30px !important; 
            font-size: 10px !important; font-weight: 600 !important; padding: 0 4px !important;
        }
        div.stButton > button[kind="primary"] { background-color: #0c7a3d !important; color: white !important; }
        
        /* [CRITICAL] Table Cell Styling for Visibility & Wrapping */
        ...
        div[data-testid="stDataFrame"] [role="gridcell"] div { 
            font-size: 16px !important; /* Slightly reduced for better fit */
            white-space: normal !important;
        }
        div[data-testid="stDataFrame"] [role="columnheader"] p { font-size: 16px !important; font-weight: 800 !important; }
        </style>
    """, unsafe_allow_html=True)

# [Dialog functions for Duplicates and Upload remain unchanged]
...
def open_upload_dialog():
    st.info("Upload drawings to integrate with the master list.")
...
def show_doc_control():
    ...
    if not os.path.exists(DB_PATH):
        st.error("System Error: 'drawing_master.xlsx' not found.")
        return
    ...
    # 2. Filters & Toolbar (Summarized for brevity)
    ...
    with rev_area:
        ...
        for i, rev in enumerate(target_revs[:7]):
            if rev_cols[i].button(f"{rev}", key=f"r_{rev}", use_container_width=True):
                st.session_state.sel_rev = rev
                st.rerun()

    st.markdown("<div style='margin-top:15px;'></div>", unsafe_allow_html=True)
    res_col, btn_area = st.columns([2, 1])
    with res_col: st.markdown(f"**Total: {len(f_df):,} records**")
    with btn_area:
    ...
    # 3. [Final Configuration] Guaranteed Visibility & Wrapping
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Description": st.column_config.TextColumn(
                "Description", 
                width="large",  # Forced to be clearly visible
                required=True
            ),
            "Remark": st.column_config.TextColumn(
                "Remark", 
                width="large"   # Expanded for wrap-around text
            ),
