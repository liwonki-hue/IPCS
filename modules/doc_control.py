...
def open_duplicate_manager(df):
    """Provides an interface to review and purge duplicate drawing entries."""
    ...
    st.warning("Action Required: Cleaning duplicates will retain only the 'latest' entry (bottom-most) for each unique Drawing Number.")
    if st.button("Confirm & Purge Duplicates", type="primary", use_container_width=True):
        ...
        st.success("Database optimization complete. Duplicates removed.")
        st.rerun()

@st.dialog("Upload & Merge Database")
def open_upload_dialog():
    """Handles the ingestion of new Excel data with Merge/Replace options."""
    st.info("Upload new support drawings to integrate with the master list or replace the entire database.")
    ...
    if new_file:
        ...
        if upload_mode == "Append (Merge)" and os.path.exists(DB_PATH):
            ...
            msg = "Data successfully merged into the master list."
        else:
            final_df = new_df
            msg = "Master database has been replaced."
...
def show_doc_control():
    ...
    if not os.path.exists(DB_PATH):
        st.error("System Error: 'drawing_master.xlsx' not detected in data directory.")
        return
    ...
    if len(dup_list) > 0:
        ...
        with c1:
            st.warning(f"⚠️ Duplicate Drawing No. Detected ({len(dup_list)} cases): {', '.join(map(str, dup_list[:3]))}...")
        with c2:
            if st.button("🛠️ Manage / Remove Duplicates", use_container_width=True):
                open_duplicate_manager(df)

    # 2. Schema Transformation (SYSTEM placed between Category and DWG. NO.)
    p_data = []
    ...
    # 3. UI - Revision Filter (Left 50% placement)
    ...
    # 4. Action Toolbar & Dashboard
    st.markdown("<div style='margin-top:15px;'></div>", unsafe_allow_html=True)
    res_col, btn_area = st.columns([2, 1]) # Compact button layout (1/3 width)
    
    with res_col:
        st.markdown(f"<div style='font-size:13px; font-weight:600; padding-top:8px;'>Total Count: {len(f_df):,} records</div>", unsafe_allow_html=True)
    ...
    with btn_area:
        ...
        with b3:
            ...
            st.download_button("📤 Export Excel", data=export_out.getvalue(), file_name="Dwg_Master_Export.xlsx", use_container_width=True)
        with b4: st.button("🖨️ Print", use_container_width=True)

    # 5. Data Viewport (Category -> SYSTEM -> DWG. NO.)
